...
替代函数的 apply() 方法.由于扩展运算符可以展开数组，所以不再需要apply()方法将数组转为函数的参数了。
...
以下 `reduce()`、`reduceRight()` 函数参数是 `fn(accumulator, item, index, Arr)`，其余都是 `fn(item, index, Arr)` 的形式。参数 `Arr` 指向调用该方法的数组。
