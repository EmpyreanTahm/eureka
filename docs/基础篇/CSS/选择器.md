...
CSS 中，选择器用来指定网页上我们想要样式化的 HTML 元素。CSS 提供了很多选择器，在选择要样式化的元素时，可以做到十分精细。本文将会简单地介绍选择器的不同使用方式，并了解它们的工作原理。

## 全局选择器

首先介绍一种较为特殊的选择器---全局选择器，由一个 `*` 代替，用于选中文档的所有内容。由于全局选择器优先级最低，单独使用时需要谨慎，一般搭配其他选择器使用。
...
类选择器以句号 `.` 开头，匹配文档中所有应用了这个 `class` 属性的元素，可以多个类选择器连用进行限制 `.notebox.danger`，结果求交集---即 `class` 既有 notebox 也有 danger 的所有元素。也可以搭配标签选择器进一步限制条件如 `p.notebox.danger`，即匹配 `class` 既有 notebox 也有 danger 的所有 `<p>` 元素。
...
在标签选择器后添加 `[]`，内部填写条件限制属性的值：

| 选择器             | 示例                             | 描述                                                                            |
| :----------------- | :------------------------------- | :------------------------------------------------------------------------------ |
| [attr]             | a[title]                         | 用于匹配**拥有** `title` 属性的所有 `<a>` 元素                                  |
| [attr=value]       | a[href="https://www.google.com"] | 用于匹配 `href` 的值为 `https://www.google.com` 的所有 `<a>` 元素               |
| [attr~=value]      | p[class~="highlighted"]          | 用于匹配 `class` 属性的一组值中，至少有一个是 highlighted 的 `<p>` 元素         |
| [attr^=value]      | li[class^="box-"]                | 用于匹配 `class` 属性值以 box- 开头的所有 `<li>` 元素                           |
| [attr$=value]      | li[class$="-box"]                | 用于匹配 `class` 属性值以 -box 结尾的所有 `<li>` 元素                           |
| [attr*=value]      | li[class*="box"]                 | 用于匹配 `class` 属性值至少有 box 子串的所有 `<li>` 元素                        |
| [attr&#x7C;=value] | html[lang&#x7C;="zh"]            | 用于匹配 `lang` 属性的值为 zh 或以 zh 开头并紧随一个连字符 `-` 的 `<html>` 元素 |

## 伪类和伪元素

伪类和伪元素数量众多，通常用于很明确的目的，合理使用他们可以减少标记文档中多余的类，让代理更灵活、更易于维护。

伪类选择器用于选择处于特定状态的元素，伪类选择器表现得如同向文档某个部分应用了一个类一样，伪类以冒号 `:` 关键字开头：
...
伪元素选择器表现得像是往标记文本中加入全新的 HTML 元素一样，而不是向现有的元素上应用类，伪元素开头为双冒号 `::`：
...
兄弟选择器 `~` 在同级的所有元素中进行匹配，相邻和不相邻都进行匹配：
...
可以用**逗号**将选择器隔开并组合起来，缩写为选择器列表，规则的效果不变：
...
CSS 代表层叠样式表，层叠的表现方式是理解 CSS 的关键。在项目过程中，会有多个应用于同一元素的规则，那么必须理解在冲突发生时应该应用哪条规则。
...
为简化优先级计算模型，可以将选择器分为：全局选择器；标签和伪元素选择器；类、属性和伪类选择器；ID 选择器；内联样式；`!important` 声明。

由于 `important` 和内联样式较为特殊，前者优先级最高，后者仅次，先进行删减。而全局选择器的优先级最低，再次进行删减，对应这剩下的三种分类，可以模拟一个三位优先级**从高到低**且**不进位**的数字 `0 0 0` 进行对应，选择器中每出现一次对应选择器，对应数位的值加 1，在进行大小判定时类似整数进行判定。
...
:::tip 总结
简单来说：`!important` > 内联 > ID > 类、属性和伪类 > 标签和伪元素 > 全局选择器
:::
